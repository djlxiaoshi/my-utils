...
export default class Request {
  ...
  private handleError(config: CustomConfig, errorMsg: string) {
    notification.error({
      message: 'Error',
      description: typeof config.showErrorMsg === 'string' ? config.showErrorMsg : errorMsg,
    });
  }

  public request(axiosConfig: AxiosRequestConfig = {}, customConfig: CustomConfig = {}) {
    ...
    return new Promise((resolve, reject) => {
      axios(finalAxiosConfig)
          ...
          if (status !== SUCCESS_CODE) {
            ...
            if (finalCustomConfig.showSuccessMsg) {
              ...
              notification.success({
                message: 'Success',
                description:
                  typeof finalCustomConfig.showSuccessMsg === 'string' ? finalCustomConfig.showSuccessMsg : data.msg,
              });
            }
            resolve(data.data);
          }
        ...
        .catch(error => {
          ...
          if (!finalCustomConfig.disableLoginCheck && status === NOT_LOGGED_IN_CODE) {
            // 登录验证
            this._loginCheck();
          }
